...
public class CollapsingImageLayout extends FrameLayout {
    ...
    private int mPoetHeaderLeftExpanded;

    private int mPoetHeaderTopExpanded;
    ...
    private float textSizeExpanded = 0;
    private float textSizeCollapsed = 0;
    private int mProfileImageAndNameTopCollapsed = 0;
    private int mProfileImageAndNameTopExpanded = 0;
    ...
    public CollapsingImageLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSubtitleLeftCollapsed = 0;
        mTitleLeftCollapsed = 100;
        mProfileImageAndNameTopCollapsed = getResources().getDimensionPixelOffset(R.dimen.subtitle_top_margin_collapsed);
        ;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int i = 0, z = getChildCount(); i < z; i++) {
            ...
            switch (child.getId()) {
                case R.id.poetProfileImageTemplate:
                    mImageLeftExpanded = child.getLeft();
                    mImageTopExpanded = child.getTop();
                    break;
                case R.id.poetNameTemplateTV:
                    mTitleLeftExpanded = child.getLeft();
                    mTitleTopExpanded = child.getTop();
                    break;
                case R.id.poetHeader:
                    View layProfileImageAndName = child.findViewById(R.id.layProfileImageAndName);
                    if (mProfileImageAndNameTopCollapsed == 0)
                        mProfileImageAndNameTopCollapsed = layProfileImageAndName.getTop() * 2;
                    mProfileImageAndNameTopExpanded = layProfileImageAndName.getTop();
                    mPoetHeaderLeftExpanded = child.getLeft();
                    mPoetHeaderTopExpanded = child.getTop();
                    if (textSizeExpanded == 0)
                        textSizeExpanded = ((TextView) child.findViewById(R.id.poetNameTemplateTV)).getTextSize();
                    textSizeCollapsed = textSizeExpanded / 2;
                    break;
            }
        }
    }
...
//                }
                ...
                if (child.getId() == R.id.poetProfileImageTemplate) {
                ...
                if (child.getId() == R.id.poetNameTemplateTV) {
                ...
                if (child.getId() == R.id.poetHeader) {


                    /*View layProfileImageAndName = child.findViewById(R.id.layProfileImageAndName);
                    if (layProfileImageAndName != null) {
                        final ViewOffsetHelper offsetHelperProfileImageAndName = getViewOffsetHelper(layProfileImageAndName);
                        int topOffset = (int) ((mProfileImageAndNameTopCollapsed - mProfileImageAndNameTopExpanded) * offsetFactor) - verticalOffset;
                        layProfileImageAndName.setPivotX(0);
                        layProfileImageAndName.setPivotY(0);
                        offsetHelperProfileImageAndName.setTopAndBottomOffset(topOffset);
                    }*/

                    View profileImage = child.findViewById(R.id.poetProfileImageTemplate);
                    try {
                        float scaleFactor = 1F - offsetFactor * .25F;
                        profileImage.setScaleX(scaleFactor);
                        profileImage.setScaleY(scaleFactor);

                        TextView poetNameTemplateTV = child.findViewById(R.id.poetNameTemplateTV);
                        poetNameTemplateTV.setPivotX(0);
                        poetNameTemplateTV.setPivotY(0);
//                    poetNameTemplateTV.setTextSize(TypedValue.COMPLEX_UNIT_PX, getTextSize(offsetFactor));
                        poetNameTemplateTV.setScaleX(scaleFactor);
                        poetNameTemplateTV.setScaleY(scaleFactor);

                        if (offsetFactor == 1) {
                            child.findViewById(R.id.poetDescriptionTemplate).setVisibility(View.INVISIBLE);
                            poetNameTemplateTV.setSingleLine(true);
                        } else {
                            child.findViewById(R.id.poetDescriptionTemplate).setVisibility(View.VISIBLE);
                            poetNameTemplateTV.setSingleLine(false);
                        }
                        offsetHelper.setTopAndBottomOffset((int) (-(mSubtitleTopCollapsed * offsetFactor) - verticalOffset));
//                    int topOffset = (int) ((mSubtitleTopCollapsed - mPoetHeaderTopExpanded) * offsetFactor) - verticalOffset;
//                    int leftOffset = (int) ((mSubtitleLeftCollapsed - mPoetHeaderLeftExpanded) * offsetFactor);
//                    child.setPivotX(0);
//                    child.setPivotY(0);
//                    offsetHelper.setTopAndBottomOffset(topOffset);
//                    offsetHelper.setLeftAndRightOffset(leftOffset);
    ...
    private float getTextSize(float offsetFactor){
        return  ((1F - offsetFactor * .25F) * textSizeExpanded);
    }
