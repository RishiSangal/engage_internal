...
public class RelativePopupWindow extends PopupWindow {
    ...
    public void showOnAnchor(@NonNull View anchor, @VerticalPosition int vertPos, @HorizontalPosition int horizPos, boolean fitInScreen) {
        showOnAnchor(anchor, vertPos, horizPos, 0, 53, fitInScreen);
    }
    ...
    public void showOnAnchor(@NonNull View anchor, @VerticalPosition int vertPos, @HorizontalPosition int horizPos, int x, int y, boolean fitInScreen) {
        ...
        switch (horizPos) {
            case HorizontalPosition.LEFT://urdu
                x -= measuredW;
                x = 53;
                break;
            ...
            case HorizontalPosition.ALIGN_LEFT:
                // Default position.
                break;
            case HorizontalPosition.RIGHT://english/hindi
                x += anchor.getWidth();//660
                x = -measuredW + anchor.getWidth() - 53;
                break;
        }
        if (fitInScreen) {
    ...
    public void setSelectableItemForeground(View view) {
        try {
            if (view != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                MyApplication.getContext().getTheme().resolveAttribute(android.R.attr.selectableItemBackground, outValue, true);
