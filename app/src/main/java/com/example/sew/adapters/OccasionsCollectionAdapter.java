...
public class OccasionsCollectionAdapter extends BaseRecyclerAdapter {
    private ArrayList<OccasionCollection> collections;
    public static final int ITEM_TYPE_HEADER = 0;
    public static final int ITEM_TYPE_CONTENT = 1;

    public OccasionsCollectionAdapter(BaseActivity activity, ArrayList<OccasionCollection> collections) {
        super(activity);
    ...
    public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
        switch (getItemViewType(position)) {
            case ITEM_TYPE_HEADER: {
                ...
                commonViewHolder.txtTitle.setText(MyHelper.getString(R.string.occasions_header_title));
                commonViewHolder.txtDescription.setText(MyHelper.getString(R.string.occasions_header_sub_title));
            }
            break;
            case ITEM_TYPE_CONTENT:
                ...
                OccasionCollection occasionCollection = collections.get(position - 1);
                collectionViewHolder.itemView.setTag(R.id.tag_data, occasionCollection);
                ImageHelper.setImage(collectionViewHolder.imgCollectionImage, occasionCollection.getImageUrl(), SHAYARI_IMAGE);
                collectionViewHolder.txtTitle.setText(occasionCollection.getTitle());
                collectionViewHolder.txtMore.setOnClickListener(view -> {
                    android.widget.PopupWindow popup = new PopupWindow(getActivity());
                    View layout = getActivity().getLayoutInflater().inflate(R.layout.menu_collections, null);
                    layout.findViewById(R.id.imgFavorite).setTag(R.id.tag_data, occasionCollection);
                    layout.findViewById(R.id.imgShare).setTag(R.id.tag_data, occasionCollection);
                    addFavoriteClick(layout.findViewById(R.id.imgFavorite), occasionCollection.getSherCollectionId(), Enums.FAV_TYPES.OCCASION.getKey(), v -> popup.dismiss());
                    updateFavoriteIcon(layout.findViewById(R.id.imgFavorite), occasionCollection.getSherCollectionId());
...
//                        }
//                    });
                    layout.findViewById(R.id.imgShare).setOnClickListener(v -> {
                        OccasionCollection occasionCollection1 = (OccasionCollection) v.getTag(R.id.tag_data);
                        MyHelper.shareContent(String.format("%s\n%s", occasionCollection1.getTitle(), occasionCollection1.getShareUrl()));
                    });
