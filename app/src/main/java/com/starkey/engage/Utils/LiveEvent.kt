...
public fun <T> EventSource<T>.observe(lifecycleOwner: LifecycleOwner, eventObserver: (T) -> Unit) {
    LiveEvent(
        this,
        lifecycleOwner,
        { event -> eventObserver.invoke(event) })
}

public inline fun <T> LifecycleOwner.observe(eventSource: EventSource<T>, noinline eventObserver: (T) -> Unit): Unit =
    eventSource.observe(this, eventObserver)